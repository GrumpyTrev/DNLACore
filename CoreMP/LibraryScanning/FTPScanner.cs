...
namespace CoreMP
{
	internal class FTPScanner
	{
		...
		private class DirectoryItem
		{
			...
			public string AbsolutePath => string.Format( "{0}/{1}", Base, Name );
		}
	}
	...
	internal class FtpRequest
	{
		...
		/// </summary>
		/// <returns></returns>
		public async Task<FtpWebResponse> MakeRequestAsync() => ( FtpWebResponse )await Request.GetResponseAsync();

		/// <summary>
		/// Abort a long running FtpWebRequest
		/// </summary>
		public void AbortRequest() => Request.Abort();
